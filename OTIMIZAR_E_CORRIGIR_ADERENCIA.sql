-- =====================================================================
-- OTIMIZAR E CORRIGIR ADERÊNCIA
-- =====================================================================
-- 1. Criar índices para performance
-- 2. Corrigir cálculo de aderência
-- 3. Adicionar dados de turno
-- =====================================================================

-- PASSO 1: Criar índices otimizados
CREATE INDEX IF NOT EXISTS idx_dados_praca_ano_semana 
ON dados_corridas (praca, ano_iso, semana_numero);

CREATE INDEX IF NOT EXISTS idx_dados_data_periodo 
ON dados_corridas (data_do_periodo) WHERE data_do_periodo IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_dados_periodo 
ON dados_corridas (periodo) WHERE periodo IS NOT NULL;

ANALYZE dados_corridas;

-- PASSO 2: Recriar dashboard_resumo COM ADERÊNCIA CORRETA E TURNO
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'dimensoes', jsonb_build_object(
      ...
      'semanas', (
        SELECT COALESCE(jsonb_agg(semana ORDER BY semana DESC), '[]'::jsonb)
        FROM (
          SELECT DISTINCT ano_iso || '-W' || LPAD(semana_numero::TEXT, 2, '0') AS semana
          FROM dados_corridas
          WHERE ano_iso IS NOT NULL AND semana_numero IS NOT NULL
            AND (v_is_admin OR (array_length(v_assigned_pracas, 1) IS NOT NULL AND praca = ANY(v_assigned_pracas)))
          ORDER BY ano_iso || '-W' || LPAD(semana_numero::TEXT, 2, '0') DESC
          LIMIT 50
        ) sub
      ),
      'pracas', (
...
-- Sucesso! Função otimizada criada.
-- Teste manualmente no dashboard
