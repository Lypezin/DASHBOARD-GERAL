-- =====================================================================
-- SUPORTAR MÚLTIPLAS SEMANAS E OTIMIZAR PERFORMANCE
-- =====================================================================
-- Modificar as funções SQL para aceitar múltiplas semanas separadas por vírgula
-- e otimizar performance com índices e melhorias nas queries
-- =====================================================================
...
BEGIN
  FOR func_record IN 
    ...
    FROM pg_proc
    WHERE proname IN ('listar_entregadores', 'listar_valores_entregadores', 'dashboard_resumo')
...
-- Atualizar listar_entregadores para aceitar múltiplas semanas (text)
...
WITH entregadores_agg AS (
  ...
  WHERE data_do_periodo IS NOT NULL
    ...
    AND (
      p_semana IS NULL 
      OR p_semana = ''
      OR (p_semana NOT LIKE '%,%' AND semana_numero = p_semana::integer)
      OR (p_semana LIKE '%,%' AND semana_numero = ANY(string_to_array(p_semana, ',')::integer[]))
    )
...
-- Atualizar listar_valores_entregadores para aceitar múltiplas semanas
...
-- Atualizar dashboard_resumo para aceitar múltiplas semanas e otimizar performance
...
-- Conceder permissões
GRANT EXECUTE ON FUNCTION public.dashboard_resumo(integer, text, text, text, text)
  TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.listar_entregadores(integer, text, text, text, text)
  TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.listar_valores_entregadores(integer, text, text, text, text)
  TO anon, authenticated, service_role;
...
-- Execute OTIMIZAR_INDICES_PERFORMANCE.sql separadamente se necessário
-- =====================================================================
