...
DECLARE
  func_record RECORD;
BEGIN
  -- Remover todas as versões de dashboard_resumo
  FOR func_record IN 
  ...
  -- Remover todas as versões de listar_entregadores
  FOR func_record IN 
    SELECT oid::regprocedure AS func_signature
    FROM pg_proc
  ...
  -- Remover todas as versões de listar_valores_entregadores
  FOR func_record IN 
    SELECT oid::regprocedure AS func_signature
    FROM pg_proc
  ...
  -- Remover todas as versões de calcular_utr
  FOR func_record IN 
    SELECT oid::regprocedure AS func_signature
    FROM pg_proc
    WHERE proname = 'calcular_utr'
    AND pronamespace = 'public'::regnamespace
  LOOP
    EXECUTE 'DROP FUNCTION IF EXISTS ' || func_record.func_signature || ' CASCADE';
  END LOOP;
...
utr_geral AS (
  SELECT 
    COALESCE(SUM(tempo_segundos), 0)::numeric / 3600 AS tempo_horas,
    COALESCE(SUM(numero_de_corridas_completadas), 0) AS corridas,
    CASE 
      WHEN COALESCE(SUM(tempo_segundos), 0) > 0 
      THEN ROUND((COALESCE(SUM(numero_de_corridas_completadas), 0)::numeric / (COALESCE(SUM(tempo_segundos), 0) / 3600)), 2)
      ELSE 0 
    END AS utr
  FROM filtered_data
),
...
SELECT jsonb_build_object(
  'geral', (SELECT jsonb_build_object(
    'tempo_horas', tempo_horas,
    'corridas', corridas,
    'utr', utr
  ) FROM utr_geral),
  'praca', COALESCE((SELECT jsonb_agg(jsonb_build_object(
    'praca', praca,
    'tempo_horas', tempo_horas,
    'corridas', corridas,
    'utr', utr
  ) ORDER BY praca) FROM utr_por_praca), '[]'::jsonb),
  'sub_praca', COALESCE((SELECT jsonb_agg(jsonb_build_object(
    'sub_praca', sub_praca,
    'tempo_horas', tempo_horas,
    'corridas', corridas,
    'utr', utr
  ) ORDER BY sub_praca) FROM utr_por_sub_praca), '[]'::jsonb),
  'origem', COALESCE((SELECT jsonb_agg(jsonb_build_object(
    'origem', origem,
    'tempo_horas', tempo_horas,
    'corridas', corridas,
    'utr', utr
  ) ORDER BY origem) FROM utr_por_origem), '[]'::jsonb),
  'turno', COALESCE((SELECT jsonb_agg(jsonb_build_object(
    'turno', turno,
    'tempo_horas', tempo_horas,
    'corridas', corridas,
    'utr', utr
  ) ORDER BY turno) FROM utr_por_turno), '[]'::jsonb)
);
$$;
