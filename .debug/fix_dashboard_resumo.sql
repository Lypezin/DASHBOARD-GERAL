-- Migration para corrigir a lógica de filtro de organização em dashboard_resumo
-- Esta correção permite que admins vejam dados de todas as organizações quando passam NULL
...
BEGIN
    ...
    -- CORREÇÃO: Se p_organization_id for NULL E usuário for admin, permitir ver todos os dados
    IF p_organization_id IS NULL AND v_is_admin = true THEN
        -- Admin com NULL explícito = ver todos os dados
        v_org_filter := NULL;
    ELSIF p_organization_id IS NOT NULL THEN
        ...
        END;
    ELSE
        -- p_organization_id é NULL mas usuário NÃO é admin
        -- Buscar organization_id do usuário
        SELECT organization_id INTO v_org_filter 
        FROM public.user_profiles 
        WHERE id = auth.uid();
        
        -- Se ainda for NULL e não for admin, bloquear acesso (segurança)
