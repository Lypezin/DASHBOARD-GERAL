-- FORÇAR atualização da função (DROP + CREATE)
-- Execute no SQL Editor

-- Primeiro, dropar a função antiga
DROP FUNCTION IF EXISTS public.dashboard_resumo(
    integer, integer, integer[], text, text, text, text, text[], text[], text[], text, date, date, text
);
...
BEGIN
    ...
    -- 2. NOVA LÓGICA: Se admin, ignorar completamente o filtro de organização
    ...
    ELSE
        ...
        -- Se ainda for NULL para não-admin, bloquear acesso
