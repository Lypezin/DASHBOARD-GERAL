-- CORREÇÃO FINAL: Tratar string vazia como NULL
...
DECLARE
    ...
    v_org_id_clean text;
BEGIN
    -- 1. Limpar organization_id: tratar string vazia como NULL
    v_org_id_clean := NULLIF(TRIM(COALESCE(p_organization_id, '')), '');
    
    -- 2. Verificar se usuário é admin/marketing/master
    SELECT (role IN ('admin', 'marketing', 'master') OR is_admin = true) 
    INTO v_is_admin 
    FROM public.user_profiles 
    WHERE id = auth.uid();
    
    -- 3. Processar organization_id
    -- Se for NULL (ou string vazia) E usuário for admin, permitir ver todos os dados
    IF v_org_id_clean IS NULL AND v_is_admin = true THEN
        v_org_filter := NULL;
    ELSIF v_org_id_clean IS NOT NULL THEN
        -- Organization ID foi passado explicitamente
        BEGIN 
            v_org_filter := v_org_id_clean::uuid; 
        EXCEPTION WHEN OTHERS THEN 
            v_org_filter := NULL; 
        END;
    ELSE
        -- Não é admin e não foi passado organization_id
        SELECT organization_id INTO v_org_filter 
        FROM public.user_profiles 
        WHERE id = auth.uid();
        
        -- Se ainda for NULL, bloquear acesso
