...
Write-Host "  Correção Segura de Error Handlers" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
git add -A
$backupName = "backup-error-handlers-$(Get-Date -Format 'yyyyMMdd-HHmmss')"
git stash push -m $backupName
Write-Host "✓ Backup criado: $backupName" -ForegroundColor Green
Write-Host ""

# ETAPA 2: Arquivos a processar (conservador - apenas padrões SIMPLES)
Write-Host "[2/5] Identificando arquivos..." -ForegroundColor Yellow

$files = @(
    # Hooks - casos simples com safeLog
...
Write-Host "✓ $($files.Count) arquivos identificados" -ForegroundColor Green
Write-Host ""

# ETAPA 3: Aplicar substituições CONSERVADORAS
Write-Host "[3/5] Aplicando correções..." -ForegroundColor Yellow
...
foreach ($filePath in $files) {
    ...
    if (-not (Test-Path $fullPath)) {
        Write-Host "  ⚠ Arquivo não encontrado: $filePath" -ForegroundColor DarkYellow
        continue
    }
    ...
    # SUBSTITUIÇÃO CONSERVADORA 1: catch (err: any) -> catch (err: unknown)
    ...
    # SUBSTITUIÇÃO CONSERVADORA 2: Apenas para padrões safeLog comuns
    # Padrão: safeLog('error', err.message, ...)
    ...
    # SUBSTITUIÇÃO CONSERVADORA 3: console.error(err.message)
    ...
    if ($content -ne $originalContent) {
        ...
        $modifiedCount++
        Write-Host "  ✓ $($filePath.Replace('src\', ''))" -ForegroundColor Green
    }
}

Write-Host ""
Write-Host "✓ $modifiedCount arquivos modificados" -ForegroundColor Green
Write-Host ""
...
if ($LASTEXITCODE -ne 0) {
    Write-Host ""
    Write-Host "❌ BUILD FALHOU! Revertendo mudanças..." -ForegroundColor Red
    Write-Host ""
    ...
    Write-Host "✓ Mudanças revertidas com sucesso" -ForegroundColor Green
    Write-Host ""
...
# ETAPA 5: Remover backup do stash (mudanças são seguras)
...
Write-Host ""
Write-Host "Próximo passo: Commit as mudanças" -ForegroundColor Cyan
