-- =====================================================================
-- BLOCO 3: CORRIGIR CÁLCULO FINAL - REPLICAR EXCEL EXATAMENTE
-- =====================================================================
-- Remove função antiga e recria com lógica correta
-- =====================================================================
  ...
  RETURN jsonb_build_object(
      ...
      FROM (
        ...
        FROM (
          SELECT
            dc.ano_iso,
            dc.semana_numero,
            -- Horas a entregar: Agrupa por (data, periodo, duracao, numero_minimo) e soma
            (
    ...
    'dia', COALESCE((
      ...
      FROM (
        ...
        FROM (
          SELECT
            EXTRACT(ISODOW FROM dc.data_do_periodo)::INTEGER AS dia_iso,
            CASE EXTRACT(ISODOW FROM dc.data_do_periodo)::INTEGER
              WHEN 1 THEN 'Segunda'
              WHEN 2 THEN 'Terça'
              WHEN 3 THEN 'Quarta'
              WHEN 4 THEN 'Quinta'
          ...
          GROUP BY EXTRACT(ISODOW FROM dc.data_do_periodo), dc.data_do_periodo
        ) calc
        ORDER BY dia_iso
      ) t
    ), '[]'::jsonb),
...
-- =====================================================================
-- SUCESSO! Agora replica exatamente o Excel:
-- 1. SELECT DISTINCT (data, periodo, duracao, numero_minimo)
-- 2. Multiplica duracao * numero_minimo para cada período único
-- 3. Soma tudo para obter horas planejadas
-- =====================================================================
