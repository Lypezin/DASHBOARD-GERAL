-- =====================================================================
-- FIX COMPLETO - DASHBOARD FUNCIONANDO + PERFORMANCE + FILTROS
-- =====================================================================
-- Resolve: timeout admin, filtro de semana, performance geral
-- =====================================================================

-- PASSO 1: Criar índices para performance (se não existirem)
-- =====================================================================
...
-- PASSO 2: Recriar função dashboard_resumo com otimizações
-- =====================================================================
...
STABLE
SECURITY DEFINER
SET statement_timeout = '120s'  -- 2 minutos para contas admin
SET work_mem = '256MB'           -- Mais memória para agregações
AS $$
DECLARE
  v_user_id UUID;
  v_is_admin BOOLEAN := true;
  v_assigned_pracas TEXT[] := ARRAY[]::TEXT[];
  v_limite_semanas INTEGER := 15;  -- Limitar semanas para performance
BEGIN
  v_user_id := auth.uid();
  ...
  -- Se admin sem filtros, aumentar limite de semanas
  ...
  RETURN jsonb_build_object(
    ...
    'sub_praca', COALESCE((
      ...
      FROM (
        ...
        GROUP BY sub_praca
        ORDER BY COALESCE(SUM(numero_de_corridas_completadas), 0) DESC
        LIMIT 50
      ) t
    ), '[]'::jsonb),
...
-- =====================================================================
-- SUCESSO! Agora teste:
-- 1. Cole este SQL completo no Supabase SQL Editor
-- 2. Execute tudo de uma vez
-- 3. Limpe o cache do navegador (Ctrl+Shift+R)
-- 4. Teste os filtros de semana
-- =====================================================================
