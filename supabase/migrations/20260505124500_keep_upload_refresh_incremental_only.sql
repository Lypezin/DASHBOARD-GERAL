create or replace function public.enqueue_mv_refresh(
...
begin
  ...
  if is_upload_reason then
    ...
    return json_build_object(
      ...
      'reason', reason,
      'upload_incremental_only', true,
      'skipped_full_worker', true,
      'worker_result', null
    );
  end if;
  ...
  should_mark := pending_count_before = 0
    or reason_normalized in ('scheduled', 'prioritized_rpc')
    or reason_normalized like 'manual:%';
