-- Fix Marketing > Entrada/Saida timeouts and premature churn.
--
-- The current overlay view checks every historical row against the small
-- incremental table. Keep the same contract, but only run the anti-join inside
-- scopes that actually have incremental rows.
...
where not exists (
    ...
    where s.organization_id = mv.organization_id
      and s.week_start_date = mv.week_start_date
...
and not exists (
    ...
    where t.organization_id = mv.organization_id
      and t.week_start_date = mv.week_start_date
      and t.praca is not distinct from mv.praca
      and t.id_entregador is not distinct from mv.id_entregador
...
begin
    ...
    -- Do not treat the current/incomplete week as proof of churn.
    -- Example: if W19 has only Monday uploaded, W18 must not be counted as exit yet.
