...
const buildCircleDasharray = (valor: number) => {
  ...
  const circumference = 2 * Math.PI * 60; // Reduced radius from 70 to 60
    ...
    <SlideWrapper isVisible={isVisible} style={{ padding: '20px 32px' }}>
      ...
      <div className="grid grid-cols-2 gap-3 flex-1 content-start">
        {itens.map((item) => (
          <div
            key={item.nome}
            className="relative flex flex-col rounded-xl bg-gradient-to-b from-slate-50 to-white border border-slate-200 p-2.5 shadow-sm"
          >
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="inline-block rounded-lg bg-blue-50 border border-blue-200 px-2 py-0.5">
                <span className="text-[0.6rem] font-medium text-blue-600 block leading-tight">Planejado</span>
                <span className="text-[0.85rem] font-bold text-blue-700 leading-tight" style={buildTimeTextStyle(item.horasPlanejadas, 0.85)}>
