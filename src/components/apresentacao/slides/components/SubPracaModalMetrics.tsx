...
export const SubPracaModalMetrics = ({ selectedItem }: { selectedItem: SubPracaComparativo }) => (
    <div className="grid grid-cols-2 gap-8 border-t border-slate-100 pt-8">
        ...
        <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-5 border border-slate-100 dark:border-slate-700/50">
            <h4 className="text-base font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-3">Variação de Performance</h4>
            <div className="flex gap-4">
                {selectedItem.variacoes.map((v) => (
                    <div key={v.label} className={`flex-1 rounded-lg p-3 text-center border ${v.positivo ? 'bg-emerald-100 dark:bg-emerald-900/30 border-emerald-200 dark:border-emerald-800/50 text-emerald-700 dark:text-emerald-400' : 'bg-rose-100 dark:bg-rose-900/30 border-rose-200 dark:border-rose-800/50 text-rose-700 dark:text-rose-400'}`}>
                        <p className="text-xs font-bold uppercase opacity-70 mb-1">{v.label}</p>
                        <p className="text-xl font-black leading-none">{v.valor}</p>
                    </div>
                ))}
            </div>
        </div>
