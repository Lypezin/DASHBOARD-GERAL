  ...
  return (
    <SlideWrapper isVisible={isVisible} style={{ padding: '35px 45px', overflow: 'visible' }}>
      <header className="text-center mb-5">
        <h2 className="text-[2.5rem] font-black leading-none tracking-wider mb-1.5 text-blue-600">ADERÊNCIA POR TURNO</h2>
      ...
      <div className="grid grid-cols-2 gap-5 flex-1" style={{ overflow: 'visible' }}>
        {itens.map((turno) => (
          <div
            key={turno.nome}
            className="rounded-[16px] bg-slate-50 border border-slate-200 px-5 py-5 flex flex-col gap-4"
            style={{ overflow: 'visible' }}
          >
            <div className="text-center" style={{ overflow: 'visible' }}>
              <h3 className="text-[26px] font-semibold uppercase tracking-wide leading-tight text-slate-900">{turno.nome}</h3>
            </div>
            ...
            <div className="flex items-center justify-between gap-4" style={{ overflow: 'visible' }}>
              {[turno.semana1, turno.semana2].map((semana, index) => (
                <div key={index} className="flex flex-col items-center gap-3 flex-1" style={{ overflow: 'visible' }}>
                  <span className="text-[18px] font-bold text-center text-slate-500">
                  ...
                  >
                    ...
                    >
                      <circle cx="80" cy="80" r="73" stroke="#e2e8f0" strokeWidth="14" fill="none" />
                      <circle
                        cx="80"
                        cy="80"
                        r="73"
                  ...
                  <div className="rounded-[10px] bg-white border border-slate-200 px-3 py-2.5 w-full flex flex-col items-center gap-1" style={{ overflow: 'visible' }}>
                    <span className="text-[15px] font-medium text-slate-500">Horas Entregues</span>
                    <span
                      className="font-bold text-emerald-600 text-center"
            ...
            <div className="grid grid-cols-3 gap-2.5 mt-auto" style={{ overflow: 'visible' }}>
              {turno.variacoes.map((variacao) => (
                <div key={variacao.label} className="rounded-[8px] bg-white border border-slate-200 px-3 py-2.5 text-center flex flex-col items-center justify-center gap-1" style={{ overflow: 'visible' }}>
                  <p className="text-[14px] font-medium text-slate-500 leading-tight">{variacao.label}</p>
                  <p
