    ...
    <SlideWrapper isVisible={isVisible} style={{ padding: '28px 36px' }}>
      ...
      <div className="grid grid-cols-2 gap-4 flex-1">
        {itens.map((turno) => (
          ...
          >
            {/* Turno name */}
            <h3 className="text-[1.25rem] font-bold uppercase tracking-wide text-slate-800 text-center mb-3">
              {turno.nome}
            </h3>
            ...
            <div className="flex items-start justify-between gap-4 flex-1">
              {[turno.semana1, turno.semana2].map((semana, index) => (
                <div key={index} className="flex flex-col items-center gap-2 flex-1">
                  <span className={`text-[0.75rem] font-bold text-center px-3 py-1 rounded-full ${index === 0 ? 'bg-slate-200 text-slate-700' : 'bg-blue-600 text-white'
                    }`}>
                  ...
                  <div className="relative flex items-center justify-center" style={{ width: '120px', height: '120px' }}>
                    ...
                    <span
                      style={buildCircleTextStyle(semana.aderencia, 1.4, 0.9)}
                  ...
                  {/* Hours delivered */}
                  <div className="rounded-lg bg-white border border-slate-200 px-3 py-2 w-full text-center">
                    <span className="text-[0.75rem] font-medium text-slate-500 block">Horas Entregues</span>
                    <span
                      className="font-bold text-emerald-600"
                      style={buildTimeTextStyle(semana.horasEntregues, 1)}
                    >
