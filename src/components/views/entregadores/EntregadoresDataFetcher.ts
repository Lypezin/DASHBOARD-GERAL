...
export async function fetchEntregadoresFallback(
  ...
  try {
    ...
    //    A MV já tem os dados agregados e é muito mais rápida (3000 linhas vs calculo em milhões).
    // 2. Se tiver filtro de "Data Início" ou a MV falhar, usar o método de agregação otimizado (lento mas preciso para histórico).
    ...
    if (usarMV) {
      try {
        ...
        if (!mvError && mvData && mvData.length > 0) {
          ...
          // Buscar dados_marketing para status e sub_praca
          ...
          for (const row of mvData) {
            ...
            entregadoresMV.push({
              ...
              dias_sem_rodar: row.dias_sem_rodar, // MV já tem calculado
              regiao_atuacao: row.regiao_atuacao,
              rodando: mktInfo?.rodando || null // Status atualizado do marketing
            });
          }
    ...
    // Se chegou aqui, ou tem filtro de Data Início, ou a MV falhou/está vazia.
