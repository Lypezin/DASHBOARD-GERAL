...
export const AnaliseMetricCards: React.FC<AnaliseMetricCardsProps> = ({ totals, taxaAceitacao, taxaCompletude, taxaRejeicao, totalHorasEntregues }) => {
    return (
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-5">
            ...
            <AnaliseHeroCard title="Aceitas" icon={CheckCircle2} value={totals.aceitas.toLocaleString('pt-BR')} accentColor="emerald" progress={{ value: taxaAceitacao, label: 'TAXA DE ACEITACAO' }} />
            <AnaliseHeroCard title="Rejeitadas" icon={XCircle} value={totals.rejeitadas.toLocaleString('pt-BR')} accentColor="rose" progress={{ value: taxaRejeicao, label: 'TAXA DE REJEICAO' }} />
            <AnaliseHeroCard title="Completadas" icon={Flag} value={totals.completadas.toLocaleString('pt-BR')} accentColor="sky" progress={{ value: taxaCompletude, label: 'TAXA DE COMPLETUDE' }} />
        </div>
    );
};
