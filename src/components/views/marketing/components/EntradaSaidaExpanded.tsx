...
const MetricBox = ({
    ...
    <div className={`rounded-xl border bg-white p-3 text-center dark:bg-slate-800/80 ${borderClass}`}>
        ...
        <div className={`mb-1 text-xl font-bold tabular-nums ${colorClass}`}>{value}</div>
        <div className="flex items-center justify-center gap-3 text-[11px]">
            <span className="flex items-center gap-1 text-slate-500">
                <Megaphone className="h-3 w-3 text-blue-400" />
                <b className="text-slate-700 dark:text-slate-200">{mkt}</b>
            </span>
            <span className="flex items-center gap-1 text-slate-500">
                <Wrench className="h-3 w-3 text-slate-400" />
                <b className="text-slate-700 dark:text-slate-200">{ops}</b>
            </span>
        </div>
    </div>
);

export const EntradaSaidaExpanded: React.FC<Props> = ({
    item,
    totalRetomada,
    entradas_ops,
    saidas_ops,
            ...
            <div className="border-t border-slate-100 bg-gradient-to-b from-slate-50/80 to-white dark:border-slate-800 dark:from-slate-800/30 dark:to-slate-900">
                <div className="space-y-5 p-6">
                    <div className="grid grid-cols-4 gap-3">
                        ...
                        <div
                            className={`rounded-xl border bg-white p-3 text-center dark:bg-slate-800/80 ${
                                (item.saidas_novos || 0) > 0
                        ...
                        >
                            ...
                            <div
                                className={`mb-1 text-xl font-bold tabular-nums ${
                                    (item.saidas_novos || 0) > 0 ? 'text-amber-700 dark:text-amber-300' : 'text-slate-400'
                                }`}
                            >
                                {item.saidas_novos || 0}
                            </div>
                    ...
                    <div className="flex items-center gap-3 pt-1">
                        <span className="mr-1 text-[11px] font-bold uppercase tracking-wider text-slate-400">
                            <Eye className="mr-1 -mt-0.5 inline-block h-3.5 w-3.5" />
                            Ver nomes:
                        </span>
                        {(item.entradas > 0 || (item.entradas_total || 0) > 0) && (
                            <MetricDetailDialog
                                ...
                                semanaIso={item.semana}
                                organizationId={organizationId}
                                praca={praca}
                                count={item.entradas_total || item.entradas}
                        ...
                        {(item.saidas > 0 || (item.saidas_total || 0) > 0) && (
                            <MetricDetailDialog
                                ...
                                organizationId={organizationId}
                                praca={praca}
                                count={item.saidas_total || item.saidas}
