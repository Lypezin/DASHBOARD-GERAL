...
export const EntradaSaidaView: React.FC<EntradaSaidaViewProps> = ({ dataInicial, dataFinal, organizationId, praca }) => {
    ...
    const handleExport = () => {
        ...
        const formattedData = data.map(item => ({
            'Semana': item.semana, 'Entradas Total': item.entradas_total, 'Entradas Mkt': item.entradas_marketing, 'Entradas Ops': item.entradas_operacional,
            'Saídas Total': item.saidas_total, 'Saídas Mkt': item.saidas_marketing, 'Saídas Ops': item.saidas_operacional, 'Saldo': item.saldo,
            'Base Ativa': item.base_ativa, 'Variação Base': item.variacao_base, 'Retomada': item.retomada_total, 'Desistências Novos': item.saidas_novos,
    ...
    if (error) return (
        <div className="rounded-2xl border border-rose-200 bg-gradient-to-br from-rose-50 to-white p-8 text-center shadow-sm dark:from-rose-950/20 dark:to-slate-900 dark:border-rose-900/50">
            <div className="mx-auto mb-4 h-14 w-14 rounded-2xl bg-rose-100 flex items-center justify-center dark:bg-rose-900/40"><Activity className="h-7 w-7 text-rose-600 dark:text-rose-400" /></div>
