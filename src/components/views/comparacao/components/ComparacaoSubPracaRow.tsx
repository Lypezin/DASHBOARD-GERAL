...
export const ComparacaoSubPracaRow = React.memo(({ subPraca, index, semanasSelecionadas, dadosPorSubPraca }: ComparacaoSubPracaRowProps) => {
    return (
        ...
        >
            <TableCell className="sticky left-0 z-10 bg-inherit py-4 pl-6 text-[13px] font-semibold text-slate-800 transition-colors group-hover:text-sky-600 dark:text-slate-200 dark:group-hover:text-sky-300 sm:pl-8">
                {subPraca}
            </TableCell>
            {semanasSelecionadas.map((_, idx) => {
                ...
                return (
                    <React.Fragment key={idx}>
                        <TableCell className="py-4 text-center font-mono text-xs text-slate-500 tabular-nums transition-colors group-hover:bg-white/50 dark:text-slate-400 dark:group-hover:bg-transparent">
                            {metric.meta}
                        </TableCell>
                        <TableCell className="py-4 text-center font-mono text-[13px] text-slate-700 tabular-nums transition-colors group-hover:bg-white/50 dark:text-slate-300 dark:group-hover:bg-transparent">
                            {metric.entregue}
                        </TableCell>
                        <TableCell className="py-4 text-center text-[13px] font-bold text-slate-900 tabular-nums transition-colors group-hover:bg-white/50 dark:text-white dark:group-hover:bg-transparent">
                            {valor.toFixed(1)}%
                        </TableCell>
