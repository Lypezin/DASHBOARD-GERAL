...
export async function exportarDashboardParaExcel(
    ...
    try {
        ...
        if (aderenciaGeral) {
            ...
            const resumoData = [{ 'Métrica': 'Horas', 'Realizado': horasRealizadas, 'Planejado': horasPlanejadas, 'Aderência': formatarPorcentagem(aderenciaGeral.aderencia_percentual || 0) }];
        ...
        if (aderenciaDia && aderenciaDia.length > 0) {
            const diasData = aderenciaDia.map(d => ({ 'Dia': d.dia_semana || d.data || 'N/A', 'Data': d.data || '-', 'Horas Realizadas': formatarHorasParaHMS((d.segundos_realizados || 0) / 3600), 'Horas Planejadas': formatarHorasParaHMS((d.segundos_planejados || 0) / 3600), 'Aderência': formatarPorcentagem(d.aderencia_percentual || 0), 'Corridas Completadas': d.corridas_completadas || 0 }));
        ...
        if (aderenciaTurno && aderenciaTurno.length > 0) {
            const turnoData = aderenciaTurno.map(t => ({ 'Turno': t.turno, 'Horas Realizadas': formatarHorasParaHMS((t.segundos_realizados || 0) / 3600), 'Horas Planejadas': formatarHorasParaHMS((t.segundos_planejados || 0) / 3600), 'Aderência': formatarPorcentagem(t.aderencia_percentual || 0), 'Corridas Completadas': t.corridas_completadas || 0 }));
        ...
        if (aderenciaSubPraca && aderenciaSubPraca.length > 0) {
            const subData = aderenciaSubPraca.map(s => ({ 'Sub-Praça': s.sub_praca, 'Horas Realizadas': formatarHorasParaHMS((s.segundos_realizados || 0) / 3600), 'Horas Planejadas': formatarHorasParaHMS((s.segundos_planejados || 0) / 3600), 'Aderência': formatarPorcentagem(s.aderencia_percentual || 0), 'Corridas Completadas': s.corridas_completadas || 0 }));
        ...
        if (aderenciaOrigem && aderenciaOrigem.length > 0) {
            const origemData = aderenciaOrigem.map(o => ({ 'Origem': o.origem, 'Horas Realizadas': formatarHorasParaHMS((o.segundos_realizados || 0) / 3600), 'Horas Planejadas': formatarHorasParaHMS((o.segundos_planejados || 0) / 3600), 'Aderência': formatarPorcentagem(o.aderencia_percentual || 0), 'Corridas Completadas': o.corridas_completadas || 0 }));
