            ...
            {msg.replyTo && (
                <div className={cn('text-[9px] px-2 py-1 rounded-t-lg border-x border-t w-full mb-[-1px] opacity-80',
                    ...
                    <div className="truncate italic opacity-90">
                        &quot;{replyTarget?.content || '...mensagem nao encontrada'}&quot;
                    </div>
                </div>
            )}
                    ...
                    <div key={`${attachment.url}-${index}`} className="mb-2 rounded-md overflow-hidden bg-black/5">
                        {attachment.type === 'image' ? (
                            ...
                            <a href={attachment.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 p-2 bg-white/10 hover:bg-white/20 transition-colors">
                                ...
                                <span className="underline opacity-90 truncate">{attachment.name || 'Anexo'}</span>
                            </a>
                        )}
                    </div>
                ))}
            ...
            {msg.reactions && Object.keys(msg.reactions).length > 0 && (
                <div className="flex flex-wrap gap-1 mt-1 -mb-1 z-10">
                    {Object.entries(msg.reactions).map(([uid, emoji]) => (
                        <span key={`${uid}-${emoji}`} className="bg-white border border-slate-100 shadow-sm rounded-full px-1 py-0.5 text-[9px]">{emoji}</span>
                    ))}
                </div>
            )}
        </div>
