...
export function EngagementLeaderboard({ userTime }: EngagementLeaderboardProps) {
    return (
        <Card>
            ...
            <CardContent>
                {userTime.length === 0 ? (
                    <div className="text-center py-10 text-muted-foreground">Nenhum dado de engajamento nas últimas 24h.</div>
                ) : (
                            ...
                            <tbody>
                                {userTime.slice(0, 10).map((stat) => (
                                    <tr key={stat.userId} className="border-b transition-colors hover:bg-muted/50">
                                        ...
                                        <td className="p-4 text-center align-middle">{stat.totalVisits}</td>
