...
export async function POST(request: Request) {
    try {
        ...
        if (!organizationId || !UUID_RE.test(organizationId)) {
            return NextResponse.json({ data: null, error: 'Organizacao invalida para consulta do DEDICADO.' }, { status: 400 });
        }
        ...
        if (result.error) {
            return NextResponse.json(
                { data: null, error: result.error.message || 'Erro ao consultar DEDICADO.', details: result.error },
                { status: 500 }
            );
        }

        return NextResponse.json({ data: result.data ?? null, error: null });
    } catch (error) {
